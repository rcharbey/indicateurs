...
for (findex in 2: length(fnames)){
  ...
  for (i in 1:commenters)
  {
    ...
    for(j in i:commenters)
    {
      ...
      if((length(a)!=0) && (length(b)!=0))
      {
        ...
        #commonEdgeNeigh =edgesCommonNeighbors(adjnew[a,], adjnew[b,], adjnew)
        ...
        if (commonN>1) #more than one common neighbor
        {
          for (k in 1:commonN)
          {
            ...
            for (kk in k : commonN)
            {
              ...
              if(adjnew[commonNindex[k],commonNindex[kk]] == 1)
              {
  ...
  #rm(U)
  data    <- data.frame(y, x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19) 
